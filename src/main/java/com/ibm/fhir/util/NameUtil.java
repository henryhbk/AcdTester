...
public class NameUtil {
	...
	public static String[] MALES = { "James", " John", " Robert", " Michael", " William", " David", " Joseph",
			...
			" Alfred", " Edwin", " Steve", " Chad", " Jackson", " Adrian" };
...
//	Native Hawaiian or Other Pacific Islander, and White
	private static List<String> RACE = new ArrayList<String>();

	private static List<String> LANGUAGE = new ArrayList<String>();

	private static int PERCENTWHITE = 61;

	private static int PERCENTHISPANIC = 18;

	private static int PERCENTBLACK = 14;

	private static int PERCENTHAWAIIAN = 1;

	private static int PERCENTISLANDER = 1;

	private static int PERCENTNATIVEAMERICAN = 2;

	private static int PERCENTENGLISH = 79;

	private static int PERCENTSPANISH = 8;

	private static int PERCENTCHINESE = 2;

	private static int PERCENTFRENCH = 1;

	private static int PERCENTCREOLE = 1;

	private static int PERCENTITALIAN = 1;

	private static int PERCENTRUSSIAN = 1;

	private static int PERCENTVIETNAMESE = 1;

	private static int PERCENTARABIC = 1;
	...
	private static void fillRaceValues() {
		for (int i = 0; i < PERCENTWHITE; i++) {
			RACE.add("White (Non-Hispanic)");
		}
		for (int i = 0; i < PERCENTHISPANIC; i++) {
			RACE.add("Hispanic");
		}
		for (int i = 0; i < PERCENTBLACK; i++) {
			RACE.add("Black");
		}
		for (int i = 0; i < PERCENTHAWAIIAN; i++) {
			RACE.add("Hawaiaan");
		}
		for (int i = 0; i < PERCENTNATIVEAMERICAN; i++) {
			RACE.add("Native American");
		}
		for (int i = 0; i < PERCENTISLANDER; i++) {
			RACE.add("Pacific Islander");
		}

	}

	private static void fillLanguages() {
		for (int i = 0; i < PERCENTENGLISH; i++) {
			LANGUAGE.add("English");
		}
		for (int i = 0; i < PERCENTSPANISH; i++) {
			LANGUAGE.add("Spanish");
		}
		for (int i = 0; i < PERCENTCHINESE; i++) {
			LANGUAGE.add("Chinese");
		}
		for (int i = 0; i < PERCENTFRENCH; i++) {
			LANGUAGE.add("French");
		}
		for (int i = 0; i < PERCENTCREOLE; i++) {
			LANGUAGE.add("Creole");
		}
		for (int i = 0; i < PERCENTITALIAN; i++) {
			LANGUAGE.add("Italian");
		}
		for (int i = 0; i < PERCENTRUSSIAN; i++) {
			LANGUAGE.add("Russian");
		}
		for (int i = 0; i < PERCENTVIETNAMESE; i++) {
			LANGUAGE.add("Vietnamese");
		}
		for (int i = 0; i < PERCENTARABIC; i++) {
			LANGUAGE.add("Arabic");
		}
	}
	...
	public static String getRandomRace() {
		...
		Random random = new Random();
		return RACE.get(random.nextInt(RACE.size()));
	}
	...
	public static String getRandomLanguage() {
		...
		Random random = new Random();
		return LANGUAGE.get(random.nextInt(LANGUAGE.size()));
	}
